<?php
/**
 * Quickshop - Grouped product add to cart
 */

if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly

global $woocommerce, $woocommerce_quickshop, $product;
...
<form action="<?php echo esc_url( $product->add_to_cart_url() ); ?>" class="cart" method="post" enctype='multipart/form-data'>
			...
			<?php foreach ( $grouped_products as $child_product ) : ?>
				<tr>
					...
					<td class="label"><label for="product-<?php echo $child_product['product']->id; ?>"><?php
						...
						// Backwards compat
						$title = ( $woocommerce_quickshop->old_wc_version ) ? $child_product['product']->get_title() : $child_product['product']->post->post_title;
						
						if ( $child_product['product']->is_visible() )
							echo '<a href="' . get_permalink( $child_product['product']->id ) . '" id="' . $child_product['product']->id . '" class="qs_grouped_product_link">' . $title . '</a>';
						else
							echo $title;
	...
	<?php if ( $quantites_required ) : ?>
		...
		<button type="submit" class="single_add_to_cart_button button qs_button qs_product_type_grouped alt" data-product_id="<?php echo $product->id; ?>"><?php echo apply_filters('single_add_to_cart_text', __('Add to cart', 'woocommerce'), $product->product_type); ?></button>
